...
class ReportService
{
    ...
    public function monthlyReport($anlagen, int $month = 0, int $year = 0, int $docType = 0, int $chartTypeToExport = 0, bool $storeDocument = true, bool $exit = true, bool $export = true): string
    {
        ...
        foreach ($anlagen as $anlage) {
            ...
            if ($export === true) $output = $this->buildMonthlyReport($anlage, $report, $docType, $chartTypeToExport, $exit);
    ...
    public function buildMonthlyReportNew(Anlage $anlage, array $report, $reportCreationDate, int $docType = 0, int $chartTypeToExport = 0, $exit = true): string
    {
        #beginn create Array for Day Values Table
        ...
        if (true) {
            ...
            for ($i = 0; $i < $daysInMonth; $i++) {
                ...
                $dayValues['PowerEvuMonth']     = ($anlage->getShowEvuDiag()) ? (float)$prArray['powerEvu'] : (float)$prArray['powerAct'];
                if ($anlage->getUseGridMeterDayData()) {
                    $dayValues['powerEGridExt'] = (float)$prArray['powerEGridExt'];
                    $dayValues['spezYield']     = $dayValues['powerEGridExt'] / $anlage->getKwPeak();
                    $dayValues['prEvuEpc']      = (float)$prArray['prEGridExt'];
                    $dayValues['prEvuDefault']  = (float)$prArray['prDefaultEGridExt'];
                } else {
                    $dayValues['powerEGridExt'] = 0;
                    $dayValues['spezYield']     = ($anlage->getShowEvuDiag()) ? (float)$prArray['powerEvu'] / $anlage->getKwPeak() : (float)$prArray['powerAct'] / $anlage->getKwPeak();
                    $dayValues['prEvuEpc']      = ($anlage->getShowEvuDiag()) ? (float)$prArray['prEvu'] : (float)$prArray['prAct'];
                    $dayValues['prEvuDefault']  = ($anlage->getShowEvuDiag()) ? (float)$prArray['prDefaultEvu'] : (float)$prArray['prDefaultAct'];
                }
                $dayValues['irradiation']       = (float)$prArray['irradiation'];
                if ($showAvailability === true)         $dayValues['plantAvailability'] = (float)$prArray['availability'];
                if ($showAvailabilitySecond === true)   $dayValues['plantAvailabilitySecond'] = -111;
                $dayValues['powerTheo']         = (float)$prArray['powerTheo'];
                $dayValues['powerExp']          = (float)$prArray['powerExp'];
                $dayValues['case5perDay']       = 0;//$report['prs'][$i]->getcase5perDay();
